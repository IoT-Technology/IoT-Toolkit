 ...
 * @author mushuwei
 */
public class MobAppConfigNode implements TkNode {
	...
	public void prePrompt(NodeContext context) {
		if (!context.getPromptData().isEmpty()) {
			...
			Stream.iterate(0, i -> i + 1).limit(configList.size()).forEach(i -> {
				MobProjectSettings settings = JsonUtils.jsonToObject(configList.get(i), MobProjectSettings.class);
	...
	public String nodePrompt() {
		return bundle.getString(NbSettingsCodeEnum.NB_MOB_APP_CONFIG.getCode() + GlobalConstants.promptSuffix) +
